...
class ClientesController(MethodView):
    ...
    def post(self):
        ...
        with mysql.cursor() as cur:
            cur.execute("INSERT INTO cadastro_cliente(nome,email,telefone,endereco,bairro,cidade,cep,senha) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)",(nome,email,telefone,endereco,bairro,cidade,cep,senha))
            cur.connection.commit()
        return redirect('/clientes')
